...
It's worth mentioning here that the large column height (approx. 17 mm) caused by large matrix area and the aggressive column-bus routing (which refers to the minimum line width and spacing) due to the smaller pixel size, generated a significant signal transmission delay due to the RC low pass filtering effect of the long metal wires. Consequently a special circuit has been studied that adds a variable delay to the hit pulse across the column that matches that of the BCID signal.
...
As we have seen above, there are two variations of the front-end circuit (figure \vpageref{fig:tj2_circuit}),  ''normal'' and ''cascode'' type. The latter in particular includes an extra cascode transistor which increases the pre-amplifier gain and consequently reduces the threshold dispersion.\\
...
The injected charge $Q_{inj}$ resolution can be calculated by:

\begin{equation}
\small
Q_{inj,res} = Q_{inj} \cdot LSB= \frac{230 \, aF}{q_{{e}^{-}}} \cdot \Delta V_{inj} = 1.4375 \frac{e^{-}}{mV} \cdot 7.03 \frac{mV}{DAC \, unit} \approx 10.1 \frac{e^{-}}{DAC \, unit}  
...
\\
The four flavors have been separately analyzed to be able to study their main differences concerning their performance and features, but the same method has been used, called \textit{s-curve method} and explained below. 
