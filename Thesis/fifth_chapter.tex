...
\item \textit{$I_{THR}$}: it controls the pre-amplifier feedback strenght and speed, so it's responsible for the output reset rate. Increasing $I_{THR}$ results to lower gain and faster return to baseline, so higher threshold. (This value is preferably set to 8nA in order to avoid high ToT slope).
\item \textit{$I_{DB}$}: 
\item \textit{$I_{TUNE}$}:
\item \textit{$I_{BIAS}$}: 
\item \textit{$V_{RESET}$}: 
...
Moreover, since a 7-bit BCID is sent to the matrix during its activity, it was important to keep short (<128) the duration of the readout sequence and to not enable too many pixels in order to not extend too much the readout frame.  
...
By this procedure, repeated




grafico dei picchi di cross talk\\
FREEXE START STOP\\
height of the induced signal\\
analog images\\
