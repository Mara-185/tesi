...
The absolute calibration of the matrix consists in characterizing the response of each pixel to a known signal, like the emission peaks of radioactive sources. By the means of the $Q_{inj}$ - ToT fit (section \ref{tot_fit}), it is posible to extrapolate the ToT signal value induced for whatever collected charge.\\
...
Bringing equation \vpageref{conversion_factor} back to mind, the conversion factors of the flavors are not those expected. In particular for the HV's flavors, it is almost the double and it could mean that the injection capacitance is greater than expected. As mentioned in section \vpageref{thresh_noise}, the prototype under test was designed in order to have the same injection capacitance for all flavor, equal to 230 aF (despite this doesn't seem to be the case).
But actually, the cause of the different results for the conversion factors is due to the expected loss of the collected charge for HV FEs (section \vpageref{improved_circuit}).
...
In the end the mean of all emission peaks from the several sources have been plotted for each frontend in order to verify the agreement between their trend and the ToT-Q relationship studied by the internal injection. \\
...
After the calibration instead, assuming the average value of injection capacitance (\textit{Mean value}) calculated in table \vpageref{tab:cap_mean}, the charge corresponding to the emission peaks have been recalculated and results are shown in figure \vpageref{fig:inj_cap_sources}.
...
\begin{comment}
Through the calibration a better agreement is therefore obtained. As we already said, this was the purpose of all this study: to obtain optimized conversion factor for all frontends to interpret results from TB, characterizing them with the conversion factor.
\end{comment}
