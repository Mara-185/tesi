...
In figure %\vpageref{fig:xtalk} 
which is the histogram of the time last between the leading edge of an hit and the trailing edge of the previous hit, and two pixels read. It's possible to see several peaks (reffering to the readout setting reported in table \vpageref{tab:ro_registers}):
...
In figure %\vpageref{fig:analog_xtalk}
...
{\includegraphics[scale=0.45]{making_hot1}}\quad
\subfigure[$I_{DB}$=60, $I_{TUNE}$=150 - Pixel starts to misbehave]
{\includegraphics[scale=0.45]{making_hot3}}\\
\subfigure[$I_{DB}$=55, $I_{TUNE}$=150 - Pixel becomes \textit{hot}]
{\includegraphics[scale=0.45]{making_hot2}}\\
...
In figure %\vpageref{fig:bias_comp} 
a comparison between the threshold dispersion respectively at -3 V and -6 V.


%reduce the tail of threshold distribution
%explanation: diminuendo la C, diminuisce l'accoppiamento e quindi l'altezza del segnale in corsstalk
