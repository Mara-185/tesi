...
In further perspectives, the target of SuperKEKB is to achieve a new record with \textit{L} = \num{6e35} $cm^{-2}s^{-1}$ and to increase the integrated luminosity from 428 $fb^{-1}$ (actual value, starting in 2019) to 50 $fb^{-1}$ (projection plot shown in figure \vpageref{lumy_projection}), in order to increase the statistics and also the hope to give an insight in some of the open questions in the SM.\\
...
%IN QUESTO CASO LASCIARE LE TRE FASI, PARLARE DELL'ATTUALE E POI DELLA NECESSIT DI UPGRADE OTTICA IN LS2 CHE è POSTICIPATA NON 26.27 MA 28 -29?
