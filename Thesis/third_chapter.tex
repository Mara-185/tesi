...
Moreover the sensor thickness has to be below 100 $\mu$m to respect the material budget constraint of 0.2\%$X_{0}$ and as consequence the depleted sensitive region should be lower than 50 $\mu$m, perfectly in agreement with the available thickness of MAPS (on the market?). 
...
\begin{comment}
The chip should then feature a pad ring facilitating the connection
with a single light cable. 
For the sake of the simplicity of the system integration, it is also specified that
the sensor is powered through on-chip voltage regulators and features a single
rather low data bandwidth (< 200MHz).
\end{comment}
