...
if __name__ == "__main__":
    ...
    with PdfPages(args.output_file) as pdf:
        ...
        y = np.arange(mean_b[0]-29.01, 250, 1)
        plt.plot(y, func_norm(y, *popt), "r-", label=f"fit {name}:\na ={ufloat(round(popt[0], 3), round(perr[0], 3))}\n"
        ...
        plt.plot(y, func_norm_a(y, *popt), "r-", label=f"fit {name}:\na ={aa}\n"
        ...
        plt.plot(y, func_norm_b(y, *popt), "r-", label=f"fit {name}:\na ={ufloat(round(popt[0], 3), round(perr[0], 3))}\n"
            f"b = {bb}\nc = {ufloat(np.around(popt[1],3),round(perr[1], 3))}\nt = {ufloat(np.around(popt[2],3),round(perr[2], 3))}")
        plt.xlim([0, 250])
        plt.ylim([0, 60])
        ...
        plt.plot(y, func_norm_t(y, *popt), "r-", label=f"fit {name}:\na ={ufloat(round(popt[0], 3), round(perr[0], 3))}\n"
            f"b = {ufloat(round(popt[1],3),round(perr[1],3))}\nc = {ufloat(np.around(popt[2],3),round(perr[2], 3))}\nt = {tt}")
        plt.xlim([0, 250])
        plt.ylim([0, 60])
