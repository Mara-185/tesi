...
It is a copy of "plot_results_scurve_pisa(fit_constraint).py". """

#   python3 -W ignore tot_fit_norm.py -i "all" -o "tot_normal_last.pdf" -totfit
...
if __name__ == "__main__":
    ...
    parser.add_argument("-i","--input_file", nargs="+",
        help="The _threshold_scan_interpreted_scurve.npz file(s). If \"all\" "
             "it takes automatically all npz files in the directory.")
    ...
    with PdfPages(args.output_file) as pdf:
        ...
        plt.suptitle(f"ToT curve (Normal, not shifted)")
        plt.xlabel("Injected charge [DAC]")
        plt.ylabel("ToT [25 ns]")
        set_integer_ticks(plt.gca().xaxis, plt.gca().yaxis)
        cb = integer_ticks_colorbar()
        ...
        if args.totfit:
            ...
            plt.ylim([0,30])
            pdf.savefig();
            plt.savefig("ToT_vs_Q_norm.png"); plt.clf()
            ...
            set_integer_ticks(plt.gca().xaxis, plt.gca().yaxis)
            pdf.savefig();
            plt.savefig("mean_on_tot_norm.png"); plt.clf()
            ...
            # charge_fit = copy.deepcopy(c_mean_sh)
            # charge_fit[23:] = np.nan
            #
            # plt.plot(charge_fit, np.linspace(0, 127, 128, endpoint=True), rasterized=True)
            # plt.suptitle(f"ToT curve mean on charge (Normal)")
            ...
            # # PLOT WITH SHIFT
            # plt.plot(c_mean_sh_clean, np.linspace(0, 127, 128, endpoint=True), rasterized=True)
            # plt.suptitle(f"ToT curve (Normal)")
            # plt.title("Mean of charge for each value of ToT", fontsize=9)
            # plt.xlabel("Injected charge [DAC]")
            # plt.ylabel("ToT [25 ns]")
            ...
            set_integer_ticks(plt.gca().xaxis, plt.gca().yaxis)
            pdf.savefig();
            plt.savefig("mean_on_charge_norm.png"); plt.clf()
            ...
            def func_norm_inv(x,a,b,c,t):
                y = (t/2)-(b/(2*a))+(x/(2*a)) + np.sqrt(((t/2)+(b/(2*a))-(x/(2*a)))**2 + (c/a))
                y_t = (t/2)-(b/(2*a)) + np.sqrt(((t/2)+(b/(2*a)))**2 + (c/a))
                return np.where(y<y_t-2, 0, y)
            ...
            plt.ylim([0, 50])
            plt.suptitle(f"ToT curve (Normal)")
            plt.title("Fit: Mean of ToT for each injected charge", fontsize=9)
            plt.xlabel("Injected charge [DAC]")
            plt.ylabel("ToT [25 ns]")
            set_integer_ticks(plt.gca().xaxis, plt.gca().yaxis)
            cb = integer_ticks_colorbar()
            ...
            pdf.savefig();
            plt.savefig("totfit_meantot_nocos_norm.png"); plt.clf()
            ...
            # FIT CUT TOT
            popt, pcov = curve_fit(func_norm_cut_mean, ch_tot_mean_sh_clean[19:], tot_mean_sh_clean[19:],
                p0 = [0.15, 2, 45, 44],bounds=([0 , -100, 0, -40], [0.3, 100,1000, 80]))
                #maxfev=10000)
            perr = np.sqrt(np.diag(pcov))
            ...
            if args.points==True:
                plt.plot(ch_tot_mean_sh_clean[19:], tot_mean_sh_clean[19:], "b.", markersize="4")
            ...
            popt, pcov = curve_fit(func_norm_cut_mean, charge_fit_clean, tot_fit_clean, p0 = [0.1, -1, 50, 40])
            #popt, pcov = curve_fit(func_norm_cut_mean, charge_fit_clean[:-1], tot_fit_clean[:-1], p0 = [0.1, -1, 50, 40])
            perr = np.sqrt(np.diag(pcov))
            ...
            plt.legend(loc="upper left")
            pdf.savefig();
            plt.savefig("tot_meancharge_nocos_norm.png"); plt.clf()
            ...
            print("FIT CHARGE MEAN CONSTRAINT C (ToT vs charge):")
            print(f"popt = {popt}")
            print(f"perr = {perr}")
            ...
            plt.legend(loc="upper left")
            pdf.savefig();
            plt.savefig("totfit_meancharge_ccos_norm.png"); plt.clf()
            ...
            popt, pcov = curve_fit(func_norm_c, ch_tot_mean_sh_clean[19:], tot_mean_sh_clean[19:],
                p0 = [0.131, 0.2, 39])
            perr = np.sqrt(np.diag(pcov))
            ...
            plt.legend(loc="upper left")
            pdf.savefig();
            plt.savefig("totfit_meantot_ccos_norm.png"); plt.clf()
            ...
            #     p0 = [0.116, 4], bounds= ([0.110, 3.9],[0.120, 4.1]))          #ORIGINAL
            perr = np.sqrt(np.diag(pcov))
            ...
            if args.points==True:
                plt.plot(charge_fit_clean[11:], tot_fit_clean[11:], "b.")
            ...
            plt.legend(loc="upper left")
            pdf.savefig();
            plt.savefig("totfit_maencharge_ccos_lin_norm.png"); plt.clf()
            ...
            error_c_fourth, c_fourth = c_err_fourth(pcov_linfourth, th_s,
                disp_threshold, popt_linfourth[0], popt_linfourth[1], popt[0],
                perr[0]) #ORIGINAL
            ...
            plt.legend(loc="upper left")
            pdf.savefig()
            plt.savefig("totfit_maencharge_ccos_all_norm_fixed.png"); plt.clf()




            # LINEAR on CHARGE NO FIXING a, b
            # popt, pcov = curve_fit(func_lin, charge_fit_clean[11:-1], tot_fit_clean[11:-1],
            #     p0 = [0.15, 2])
            # perr = np.sqrt(np.diag(pcov))
            #
            ...
            # plt.legend(loc="upper left")
            # pdf.savefig();
            # plt.savefig("totfit_maencharge_ccos_lin_norm2.png"); plt.clf()
            #
            # perr_linfourth2 = copy.deepcopy(perr)
            # popt_linfourth2 = copy.deepcopy(popt)
            # pcov_linfourth2 = copy.deepcopy(pcov)
            #
            # a_min = popt_linfourth2[0]-perr_linfourth2[0]
            # a_max = popt_linfourth2[0]+perr_linfourth2[0]
            # b_min = popt_linfourth2[1]-perr_linfourth2[1]
            # b_max = popt_linfourth2[1]+perr_linfourth2[1]
            #
            # #ALL
            # popt, pcov = curve_fit(func_norm_c, charge_fit_clean[:-1], tot_fit_clean[:-1],
            #      p0=(popt_linfourth2[0], popt_linfourth2[1], 40),
            #      bounds=([a_min ,b_min,20],[a_max, b_max,70]))
            # perr = np.sqrt(np.diag(pcov))
            #
            # plt.pcolormesh(
            #     charge_shifted, np.linspace(-0.5, 127.5, 128, endpoint=True),
            ...
            # y = np.arange(th_s-0.01, 189, 1)
            #
            # error_c_fourth2, c_fourth2 = c_err_third(pcov, th_s, disp_threshold, popt[0], popt[1], popt[2])
            # print(f"C +/- dC = {c_fourth2} +/- {error_c_fourth2}")
            #
            # popt_fourth2 = copy.deepcopy(popt)
            # pcov_fourth2 = copy.deepcopy(pcov)
            #
            # rel_a = round((perr[0]/popt[0])*100,1)
            # rel_b = round((perr[1]/popt[1])*100,1)
            # rel_c = round((error_c_fourth2/c_fourth2)*100, 1)
            # rel_t = round((perr[2]/popt[2])*100,1)
            # print(f"Relative error (a, b, c, t): {rel_a}%, {rel_b}%, {rel_c}%, {rel_t}%")
            #
            # plt.plot(y, func_norm_c(y, *popt), "r-",
            #     label=f"Fixed:\n$a ={ufloat(popt[0],perr[0]):L}$\n"
            #     f"$b = {ufloat(popt[1],perr[1]):L}$\n\nCalculated:\n"
            #     f"$c = {ufloat(c_fourth, error_c_fourth):L}$\n\nFit:\n$t = {ufloat(popt[2],perr[2]):L}$")
            #
            ...
            # plt.ylim([0, 50])
            # plt.suptitle(f"ToT curve (Normal)")
            # plt.title("Fit with a,b no fixed and constraint on c: Mean of charge for each ToT", fontsize=9)
            # plt.xlabel("Injected charge [DAC]")
            # plt.ylabel("ToT [25 ns]")
            # set_integer_ticks(plt.gca().xaxis, plt.gca().yaxis)
            # cb = integer_ticks_colorbar()
            ...
            # plt.legend(loc="upper left")
            # pdf.savefig()
            # plt.savefig("totfit_maencharge_ccos_all_norm2.png"); plt.clf()
            ...
            popt, pcov = curve_fit(func_fixed_params_t, ch_tot_mean_sh_clean[19:], tot_mean_sh_clean[19:],
                                    p0=(40,))
            perr = np.sqrt(np.diag(pcov))
            ...
            plt.legend(loc="upper left")
            pdf.savefig()
            plt.savefig("totfit_maentot_ccos_all_norm_fixed.png"); plt.clf()





            # LINEAR on TOT NO FIXING a, b
            # popt, pcov = curve_fit(func_lin, ch_tot_mean_sh_clean[50:], tot_mean_sh_clean[50:],
            #     p0 = [0.15, 2])               #ORIGINAL
            # perr = np.sqrt(np.diag(pcov))
            #
            ...
            # plt.legend(loc="upper left")
            # pdf.savefig();
            # plt.savefig("totfit_maentot_ccos_lin_norm2.png"); plt.clf()
            #
            # perr_linfourth_t2 = copy.deepcopy(perr)
            # popt_linfourth_t2 = copy.deepcopy(popt)
            # pcov_linfourth_t2 = copy.deepcopy(pcov)
            #
            # a_min = popt_linfourth_t2[0]-perr_linfourth_t2[0]
            # a_max = popt_linfourth_t2[0]+perr_linfourth_t2[0]
            # b_min = popt_linfourth_t2[1]-perr_linfourth_t2[1]
            # b_max = popt_linfourth_t2[1]+perr_linfourth_t2[1]
            #
            # #ALL
            # popt, pcov = curve_fit(func_norm_c, ch_tot_mean_sh_clean[19:], tot_mean_sh_clean[19:],
            #      p0=(popt_linfourth_t2[0], popt_linfourth_t2[1], 40),
            #      bounds=([a_min ,b_min,20],[a_max, b_max,70]))
            # perr = np.sqrt(np.diag(pcov))
            #
            # plt.pcolormesh(
            ...
            # y = np.arange(40, 189, 1)
            #
            # error_c_fourth_t2, c_fourth_t2 = c_err_third(pcov, th_s, disp_threshold, popt[0], popt[1], popt[2])
            # print(f"C +/- dC = {c_fourth_t2} +/- {error_c_fourth_t2}")
            #
            # popt_fourth_t2 = copy.deepcopy(popt)
            # pcov_fourth_t2 = copy.deepcopy(pcov)
            #
            # rel_a = round((perr[0]/popt[0])*100,1)
            # rel_b = round((perr[1]/popt[1])*100,1)
            # rel_c = round((error_c_fourth_t2/c_fourth_t2)*100, 1)
            # rel_t = round((perr[2]/popt[2])*100,1)
            # print(f"Relative error (a, b, c, t): {rel_a}%, {rel_b}%, {rel_c}%, {rel_t}%")
            #
            # plt.plot(y, func_norm_c(y, *popt), "r-",
            #     label=f"Fixed:\n$a ={ufloat(popt[0],perr[0]):L}$\n"
            #     f"$b = {ufloat(popt[1],perr[1]):L}$\n\nCalculated:\n"
            #     f"$c = {ufloat(c_fourth_t2, error_c_fourth_t2):L}$\n\nFit:\n$t = {ufloat(popt[2],perr[2]):L}$")
            #
            # if args.points==True:
            #     plt.plot(ch_tot_mean_sh_clean[19:], tot_mean_sh_clean[19:], "b.", markersize=4)
            #
            ...
            # plt.ylim([0, 50])
            # plt.suptitle(f"ToT curve (Normal)")
            # plt.title("Fit with a,b no fixed and constraint on c: Mean of TOT for each charge", fontsize=9)
            # plt.xlabel("Injected charge [DAC]")
            # plt.ylabel("ToT [25 ns]")
            # set_integer_ticks(plt.gca().xaxis, plt.gca().yaxis)
            # cb = integer_ticks_colorbar()
            ...
            # plt.legend(loc="upper left")
            # pdf.savefig()
            # plt.savefig("totfit_maentot_ccos_all_norm2.png"); plt.clf()
            ...
            # error_charge4, charge4 = dx_ev_fourth_wo_c(popt_linfourth[0], popt_linfourth[1], popt_fourth[0], pcov_linfourth, disp_threshold, th_s, pcov_fourth) # ORIGINAL
            # print(f"ERROR ON CHARGE FROM TOT DISPERSION (FE55) FOURTH {error_charge4, charge4}")



            print(popt, pcov)
            error_charge4_th, charge4_th = dx_ev_wo_c(popt[0], popt[1], popt[2], pcov, disp_threshold, th_s)
            print(f"ERROR ON CHARGE FROM TOT DISPERSION (FE55) FOURTH {error_charge4_th, charge4_th}")
            ...
            # tot_fou, detot_fou = dtot3_wo_c(popt_linfourth[0], popt_linfourth[1], popt_fourth[0], th_s, disp_threshold, pcov_linfourth, pcov_fourth, error_charge4)   #ORIGINAL
            # print(f"TOT FOURTH: {tot_fou, detot_fou}")
            tot_fou_th, detot_fou_th = dtot2_wo_c(popt[0], popt[1], popt[2], th_s, disp_threshold, pcov, error_charge4_th)
            print(f"TOT FOURTH: {tot_fou_th, detot_fou_th}")
        ...
        #     # lc = min(col_stop-col_start - 1, lc - col_start)
            if name=="Normal":
                ...
                # 1
                plt.hist2d(occupancy_charges[100,60,29:].reshape(-1)-29,
                        occupancy_npz[100,60,29:].reshape(-1),
                        bins=[198, 150], range=[charge_dac_range, [0, 1.5]],
                        cmin=1, rasterized=True)  # Necessary for quick save and view in PDF
                ...
                popt, pcov = curve_fit(sfit, occupancy_charges[100,60,29:].reshape(-1)-29, occupancy_npz[100,60,29:].reshape(-1))
                perr = np.sqrt(np.diag(pcov))
                ...
                # 2
                plt.hist2d(occupancy_charges[100,400,:].reshape(-1),
                        occupancy_npz[100,400,:].reshape(-1),
                        bins=[198, 150], range=[charge_dac_range, [0, 1.5]],
                        cmin=1, rasterized=True)  # Necessary for quick save and view in PDF
                plt.suptitle(f"S-Curve (100,400)")
                plt.xlabel("Injected charge [DAC]")
                plt.ylabel("Occupancy")
                set_integer_ticks(plt.gca().xaxis)
                cb = integer_ticks_colorbar()
                ...
                popt, pcov = curve_fit(sfit, occupancy_charges[100,400,:].reshape(-1), occupancy_npz[100,400,:].reshape(-1))
                perr = np.sqrt(np.diag(pcov))
                ...
                if args.thfit:
                    ...
                    for i in tqdm(range(0,224), unit="column"):
                        for j in range(0,512):
                            popt, pcov = curve_fit(sfit, occupancy_charges[i,j,:].reshape(-1), occupancy_npz[i,j,:].reshape(-1))
                    ...
                    plt.pcolormesh(edges[:224], edges, th_fit.transpose(),
                                   rasterized=True)
                    plt.suptitle("Threshold map")
                    plt.xlabel("Column")
                    plt.ylabel("Row")
                    ...
                    # Noise map
                    plt.axes((0.125, 0.11, 0.775, 0.72))
                    plt.pcolormesh(edges[:224], edges, nois_fit.transpose(),
                                   rasterized=True)  # Necessary for quick save and view in PDF
                    plt.suptitle("Noise map")
                    plt.xlabel("Column")
                    plt.ylabel("Row")
                    ...
                    if args.thfile!=None:
                        ...
                        with np.load(args.thfile) as thdata:
                            ...
                            # m = int(np.ceil(nois_fit.max(initial=0, where=np.isfinite(noise)))) + 1
                            # noise_mean = ufloat(np.mean(nois_fit[nois_fit>0]), np.std(nois_fit[nois_fit>0], ddof=1))
                            plt.hist(nois_fit.reshape(-1), bins=50)#, range=[0, m],
                                     #label=f"{name} ${noise_mean:L}$", histtype='step', color=f"C{i}")
                            plt.suptitle(f"Noise distribution")
                            plt.xlabel("Noise [DAC]")
                            plt.ylabel("Pixels / bin")
