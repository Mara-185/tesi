...
if __name__ == "__main__":
    ...
    with PdfPages(args.output_file) as pdf:
        ...
        if args.totfit:
            ...
            plt.plot(y, func_norm_cut_mean(y, *popt), "r-", label=f"fit Normal:\n$a ={ufloat(popt[0],perr[0]):L}$\n"
            ...
            pdf.savefig();
            plt.savefig("Totfitnormal(200)_nocos_totmean.png"); plt.clf()
            ...
            # plt.legend(loc="upper left")
            # pdf.savefig();
            # plt.savefig("Totfitnormal(200)_nocos_totmean_cut200.png"); plt.clf()
            #
            # print(f"COVARIANCE:\n {pcov}")
            ...
            plt.legend(loc="upper left")
            pdf.savefig();
            plt.savefig("Totfitnormal(200)_nocos_totmost.png"); plt.clf()
            ...
            plt.legend(loc="upper left")
            pdf.savefig();
            plt.savefig("Totfitnormal(200)_nocos_chmean.png"); plt.clf()
            ...
            cb.set_label("Hits / bin")
            pdf.savefig();
            plt.savefig("Totnormal(200)_inv.png"); plt.clf()
            ...
            #y = np.arange(-2, 128, 1)
            plt.plot(y, func_norm_inv(y, *popt), "r-", label=f"fit Normal:\n$a ={ufloat(popt[0], perr[0]):L}$\n"
                f"$b = {ufloat(popt[1],perr[1]):L}$\n$c = {ufloat(popt[2],perr[2]):L}$\n$t = {ufloat(popt[3], perr[3]):L}$")
            ...
            pdf.savefig();
            plt.savefig("Totfitnormal(200)_nocos_totmean_inv.png"); plt.clf()
            ...
            plt.legend(loc="upper right")
            pdf.savefig();
            plt.savefig("Totfitnormal(200)_nocos_chmean_inv.png"); plt.clf()
            ...
            y = np.arange(40, 189, 1)
            plt.plot(y, func_norm_c(y, *popt), "r-", label=f"fit Normal:\n$a ={ufloat(popt[0],perr[0]):L}$\n"
            ...
            cb.set_label("Hits / bin")
            plt.legend(loc="upper left")
            plt.savefig("Totfitnormal(200)_cosc_totmean.png")
            pdf.savefig(); plt.clf()
            ...
            plt.legend(loc="upper left")
            pdf.savefig();
            plt.savefig("Totfitnormal(200)_cosc_chmean.png"); plt.clf()
            ...
            y = np.linspace(-0.5, 127.5, 128, endpoint=True)
            #y = np.arange(-2, 128, 1)
            plt.plot(y, func_norm_c_inv(y, *popt), "r-", label=f"fit Normal:\n$a ={ufloat(popt[0],perr[0]):L}$\n"
                f"$b = {ufloat(popt[1],perr[1]):L}$\n$c = {ufloat(cc,dcc):L}$\n$t = {ufloat(popt[2],perr[2]):L}$")
            ...
            plt.legend(loc="upper right")
            pdf.savefig();
            plt.savefig("Totfitnormal(200)_cosc_totmean_inv.png"); plt.clf()
            ...
            plt.legend(loc="upper right")
            pdf.savefig();
            plt.savefig("Totfitnormal(200)_cosc_chmean_inv.png"); plt.clf()
    ...
    ################################################################################
            ...
            # def func_lin(x,a,b):
            #     return a*x+b
            # popt, pcov = curve_fit(func_lin, charge_dac_bins2[72:], tot_mean_shift[72:],
            #     p0 = [0.15, 2])
            # perr = np.sqrt(np.diag(pcov))
            #
            # plt.pcolormesh(
            ...
            # plt.plot(y, func_norm(y, *popt, 0, -1000), "r-") #, label=f"fit:\na ={ufloat(round(popt[0], 3), round(perr[0], 3))}\n"
            ...
            # plt.xlim([0, 250])
            # plt.ylim([0, 60])
            # plt.suptitle(f"ToT curve (Normal)[lin]")
            ...
            #ALL
            # def func_fixed_params(x, c):
            #     # return func_norm(x, popt_lin[0], popt_lin[1], c, t)
            #     return func_norm_t(x, popt_lin[0], popt_lin[1], c)
            #
            # popt, pcov = curve_fit(func_fixed_params, charge_dac_bins2, tot_mean_shift,
            #                         p0=(40,))
            #
            # perr = np.sqrt(np.diag(pcov))
            #
            ...
            #
            # y = np.arange(mean_b[0]-29.01, 189, 1)
            # t = (th_s - popt[0]/(popt_lin[0]*th_s + popt_lin[1]))
            # plt.plot(y, func_fixed_params(y, *popt), "r-", label=f"fit:\n$a ={ufloat(popt_lin[0], perr_lin[0]):L}$\n"
            #     f"$b = {ufloat(popt_lin[1],perr_lin[1]):L}$\n$c = {ufloat(popt[0],perr[0]):L}$\n$t = {t}$")
            #
            # # plt.plot(charge_dac_bins2, tot_mean_shift, "b.", label="Data")
            # # plt.plot(charge_dac_bins17, tot_pos_shift, "g.", label="Data")
            #
            # plt.xlim([0, 250])
            # plt.ylim([0, 60])
            # plt.suptitle(f"ToT curve (Normal)")
            ...
            #     tot.transpose(), vmin=1, cmap=VIRIDIS_WHITE_UNDER, rasterized=True)  # Necessary for quick save and view in PDF
            #
            # aa = (popt[1])/(th_s*(th_s-popt[2])) - (popt[0])/(th_s)
            # # daa =
            # #a = (c)/(th_s*(th_s-t)) - (b)/(th_s)
            #
            ...
            #
            # # (c)/(th_s-t) - a*th_s
            # bb = (popt[1])/(th_s-popt[2]) - popt[0]*th_s
            # # dbb =
            #
            #
            ...
            #
            # #  th_s - (c)/(a*th_s + b)
            # tt = th_s - (popt[2])/(popt[0]*th_s + popt[1])
            # #dtt =
            #
        ...
        #     # lc = min(col_stop-col_start - 1, lc - col_start)
            if name=="Normal":
                ...
                # 1
                plt.hist2d(occupancy_charges[10,100,:].reshape(-1),
                        occupancy_npz[10,100,:].reshape(-1),
                        bins=[198, 150], range=[charge_dac_range, [0, 1.5]],
                        cmin=1, rasterized=True)  # Necessary for quick save and view in PDF
                ...
                popt, pcov = curve_fit(sfit, occupancy_charges[10,100,:].reshape(-1), occupancy_npz[10,100,:].reshape(-1))
                ...
                # 2
                plt.hist2d(occupancy_charges[10,400,:].reshape(-1),
                        occupancy_npz[10,400,:].reshape(-1),
                        bins=[198, 150], range=[charge_dac_range, [0, 1.5]],
                        cmin=1, rasterized=True)  # Necessary for quick save and view in PDF
                ...
                popt, pcov = curve_fit(sfit, occupancy_charges[10,400,:].reshape(-1), occupancy_npz[10,400,:].reshape(-1))
