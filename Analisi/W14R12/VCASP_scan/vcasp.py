...
def main(input_file,p0, overwrite=False):
    ...
    with PdfPages(output_file) as pdf:
        ...
        vcasp = cfg.get('configuration_out.chip.registers.VCASP')
        pdf.savefig(); plt.clf()
...
##############################################
            ...
            popt, pcov = curve_fit(gauss, bin_center, bin_height, p0 = p_0[i-1], bounds=(low, up))
            print(name, vcasp)
            print(*popt)
            mean[i] = popt[1]
            ...
            with open(f"th_fitresults_(VH={the_vh},VCASP={vcasp})[{name}].txt", "w") as outf:
...
###################################################
        ...
        tots = []
        # ToT vs injected charge as 2D histogram
        for i, ((fc, lc, name), hist) in enumerate(zip(chain([(0, 511, 'All FEs')], FRONTENDS), tot_hist)):
            if fc >= col_stop or lc < col_start:
                continue
            plt.pcolormesh(
            ...
            #
            #     popt, pcov = curve_fit(func_casc, charge_dac_bins, tot_mean, p0 = [0.15, 2, 10, 55],bounds=(low, up), maxfev=10000)
            #     perr = np.sqrt(np.diag(pcov))
            #
            #     print(*popt)
            #     print(*perr)
            ...
            #         hist.transpose(), vmin=1, cmap=VIRIDIS_WHITE_UNDER, rasterized=True)  # Necessary for quick save and view in PDF
            #
            #     y = np.arange(mean[i-1]-0.1, 250, 1)
            ...
            #     cb = integer_ticks_colorbar()
            #     cb.set_label("Hits / bin")
            #     #plt.text(20, 23, f"a ={ufloat(round(popt[0], 3), round(perr[0], 3))}\nb = {ufloat(round(popt[1],3),round(perr[1],3))}\nc = {ufloat(round(popt[2],3),round(perr[2], 3))}\nt = {ufloat(np.around(popt[3],3),round(popt[3], 3))}", bbox = {"facecolor":"white"})#, ha="right", va="top")
            #     pdf.savefig(); plt.clf()
        ...
        plt.close()
        return mean[0], mean[1], vcasp, occupancy_edges[2][:-1], tots
...
if __name__ == "__main__":
    ...
    if args.input_file:  # If anything was given on the command line
        ...
        files.extend(glob.glob("*_threshold_scan_interpreted.h5"))
    files.sort()
    ...
    for fp in tqdm(files, unit="file"):
        try:
            if i<15:
                ...
                p_0 = [[5, 100, 3], [5, 80, 3]]
            mean_n, mean_c, Vcasp, ch, totsnc = main(fp,p_0, args.overwrite)
            means_normal.append(mean_n)
            means_casc.append(mean_c)
            vcasp_all.append(Vcasp)
            chs.append(ch)
            tots_n.append(totsnc[0])
            tots_c.append(totsnc[1])
            i+=1
        except Exception:
            print(traceback.format_exc())

    with open("TH_vs_VCASP.txt", "w") as outf:
        print("VCASP:THN:THC", file=outf)
        for vc, thn, thc in zip(vcasp_all,means_normal, means_casc):
            print(vc, thn, thc, file=outf)

    plt.plot(vcasp_all, means_normal, "bo", vcasp_all, means_casc, "ro")
    plt.title("TB settings")
    plt.suptitle("Threshold vs VCASP")
    plt.xlabel("VCASP [DAC unit]")
    plt.ylabel("Threshold [DAC unit]")
    plt.grid()
    plt.legend(("Normal FE", "Cascode FE"))
    plt.savefig("TH_vs_VCASP(point).pdf")
    plt.clf()

    for n in range(0, 15, 1):
        a=vcasp_all[n]
        plt.plot(chs[n], tots_n[n], label=f"VCASP={vcasp_all[n]}")
    plt.title("TB settings, Normal FE")
    plt.suptitle("ToT curves for different VCASP")
    plt.xlabel("True injected charge [DAC unit]")
    plt.ylabel("ToT [25 ns]")
    plt.grid()
    plt.legend(fontsize=8)
    plt.savefig("tot_curves_vcasp(normal).pdf")
    plt.clf()

    for n in range(0, 15, 1):
        a=vcasp_all[n]
        plt.plot(chs[n], tots_c[n], label=f"VCASP={vcasp_all[n]}")
    plt.title("TB settings, Cascode FE")
    plt.suptitle("ToT curves for different VCASP")
    plt.xlabel("True injected charge [DAC unit]")
    plt.ylabel("ToT [25 ns]")
    plt.grid()
    plt.legend(fontsize=8)
    plt.savefig("tot_curves_vcasp(cascode).pdf")
